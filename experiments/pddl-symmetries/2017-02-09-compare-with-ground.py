...
def main(revisions=None):
    ...
    exp.add_fetcher('data/2017-02-09-pddlsymmetries-eval')

    def rename(props):
        props[generator_count_grounded] = props.get('generators_count', None)
        props[generator_orders] = props.get('generators_orders', None)
        return props

    exp.add_fetcher(os.path.expanduser('~/experiments/structural-symmetries-pruning/2017-02-09-lmcut-oss-dks-stabilizeinit-eval'), filter=[rename])

    exp.add_absolute_report_step(attributes=attributes,filter=[check_completion])

    exp.run_steps()

main(revisions=['68c8df4876ea'])
