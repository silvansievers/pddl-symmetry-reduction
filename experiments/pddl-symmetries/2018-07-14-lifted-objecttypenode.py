...
def main(revisions=None):
    ...
    configs = {
        IssueConfig('translate-symm-stabgoal-stabinit-objtypenodes', ['--translate-options', '--compute-symmetries', '--bliss-time-limit', '300', '--compute-group-order', '--add-object-type-nodes'], driver_options=['--translate', '--translate-time-limit', '30m', '--translate-memory-limit', '2G']),
        IssueConfig('translate-symm-stabinit-objtypenodes', ['--translate-options', '--compute-symmetries', '--do-not-stabilize-goal', '--bliss-time-limit', '300', '--compute-group-order', '--add-object-type-nodes'], driver_options=['--translate', '--translate-time-limit', '30m', '--translate-memory-limit', '2G']),
        IssueConfig('translate-symm-stabgoal-objtypenodes', ['--translate-options', '--compute-symmetries', '--do-not-stabilize-initial-state', '--bliss-time-limit', '300', '--compute-group-order', '--add-object-type-nodes'], driver_options=['--translate', '--translate-time-limit', '30m', '--translate-memory-limit', '2G']),
        IssueConfig('translate-symm-objtypenodes', ['--translate-options', '--compute-symmetries', '--do-not-stabilize-goal', '--do-not-stabilize-initial-state', '--bliss-time-limit', '300', '--compute-group-order', '--add-object-type-nodes'], driver_options=['--translate', '--translate-time-limit', '30m', '--translate-memory-limit', '2G']),
    }
    ...
    algorithm_nicks = [
        'translate-symm-stabgoal-stabinit-objtypenodes',
        'translate-symm-stabinit-objtypenodes',
        'translate-symm-stabgoal-objtypenodes',
        'translate-symm-objtypenodes',
    ]
    ...
    exp.add_fetcher('data/2018-07-12-lifted-eval',filter_algorithm=['{}-{}'.format(OLD_REV, x) for x in other_algorithm_nicks])

    exp.add_report(
        ComparativeReport(
            algorithm_pairs=[('{}-{}'.format(OLD_REV, other_algorithm_nicks[key]), '{}-{}'.format(REVISION, nick)) for key, nick in enumerate(algorithm_nicks)],
