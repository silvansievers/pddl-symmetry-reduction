...
vector<int> g_dom_sum_by_var;
vector<int> g_var_by_val;
...
void read_symmetry_generators(
    ...
    if (count) {
        ...
        g_permutation_length = variables.size() + size;
...
RootTask::RootTask(istream &in) {
    ...
    read_symmetry_generators(in, variables);
